...
import { merkle, makeClientId, serializeClock, deserializeClock,
         makeClock, setClock, Timestamp } from './src/crdt';
...
async function runCrdtTests(): Promise<TestResult[]> {
  ...
  try {
    ...
    results.push({ name: 'makeClientId()', ok: id.length === 16, detail: `"${id}"` });
  } catch (e: unknown) {
  ...
  try {
    ...
    results.push({ name: 'Timestamp.send()', ok: ts !== null, detail: String(ts) });
  } catch (e: unknown) {
  ...
  try {
    ...
    results.push({ name: 'serializeClock / deserializeClock', ok: false, detail: String(e) });
  }
  ...
  try {
    ...
    results.push({ name: 'merkle.insert / diff', ok: diffTime !== null, detail: `diff=${diffTime}` });
  } catch (e: unknown) {
    results.push({ name: 'merkle.insert / diff', ok: false, detail: String(e) });
  }

  try {
    const bytes = Message.encodeToBinary({ dataset: 'transactions', row: 'r1', column: 'amount', value: 'N:5000' });
    const dec = Message.decodeFromBinary(bytes);
    results.push({ name: 'Message protobuf round-trip', ok: dec.value === 'N:5000', detail: `bytes=${bytes.length}` });
  } catch (e: unknown) {
    results.push({ name: 'Message protobuf round-trip', ok: false, detail: String(e) });
  }

  try {
    const reqBytes = SyncRequest.encodeToBinary({
      messages: [{ timestamp: '2024-01-01T00:00:00.000Z-0000-0123456789ABCDEF', isEncrypted: false,
        content: Message.encodeToBinary({ dataset: 'accounts', row: 'r1', column: 'name', value: 'S:Test' }) }],
      fileId: 'file-001', groupId: 'group-001', keyId: '', since: '2024-01-01T00:00:00.000Z-0000-0000000000000000',
    });
    const req = SyncRequest.decodeFromBinary(reqBytes);
    results.push({ name: 'SyncRequest protobuf', ok: req.fileId === 'file-001', detail: `bytes=${reqBytes.length}` });
  } catch (e: unknown) {
    results.push({ name: 'SyncRequest protobuf', ok: false, detail: String(e) });
  }
  ...
  try {
    ...
    const msg = { timestamp: ts, dataset: 'transactions', row: 'tx1', column: 'amount', value: 'N:1500' };
    const requestBytes = await encode('group-1', 'file-1', ts, [msg]);
    const responseBytes = SyncResponse.encodeToBinary({
      messages: [{ timestamp: ts.toString(), isEncrypted: false,
        content: Message.encodeToBinary({ dataset: msg.dataset, row: msg.row, column: msg.column, value: String(msg.value) }) }],
      merkle: '{}',
    });
    const { messages: decoded } = await decode(responseBytes);
    const ok = requestBytes.length > 0 && decoded[0]?.value === 'N:1500';
    results.push({ name: 'encoder encode/decode', ok, detail: `req=${requestBytes.length}b` });
  } catch (e: unknown) {
    results.push({ name: 'encoder encode/decode', ok: false, detail: String(e) });
  }
...
async function runDbTests(): Promise<TestResult[]> {
  ...
  try {
    ...
    results.push({ name: 'createAccount() + getAccounts()', ok: false, detail: String(e) });
  }
  ...
  try {
    ...
    results.push({ name: 'createCategoryGroup() + createCategory()', ok: false, detail: String(e) });
  }
  ...
  try {
    ...
    const txId = await addTransaction({ acct: acctId, date: 20250302, amount: -5000 });
    ...
    results.push({ name: 'addTransaction() + getTransactions()', ok: false, detail: String(e) });
  }
    ...
    <ScrollView style={styles.container}>
      ...
      {running ? (
        ...
        <>
          ...
          {crdtResults.map((r, i) => <ResultCard key={i} result={r} />)}

          <Text style={styles.section}>SQLite + CRDT writes</Text>
          {dbResults.map((r, i) => <ResultCard key={i} result={r} />)}
        </>
      )}
    </ScrollView>
  );
...
function ResultCard({ result: r }: { result: TestResult }) {
  return (
    <View style={[styles.card, r.ok ? styles.cardOk : styles.cardFail]}>
      <Text style={styles.cardTitle}>{r.ok ? '✓' : '✗'} {r.name}</Text>
...
const styles = StyleSheet.create({
  ...
  section: { color: '#64748b', fontSize: 12, fontWeight: '700', textTransform: 'uppercase',
