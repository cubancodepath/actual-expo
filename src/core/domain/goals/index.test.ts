...
describe("inferGoalFromDef", () => {
  ...
  it("simple refill with daily limit → longGoal true", () => {
    const def = JSON.stringify([
      {
        type: "simple",
        limit: { amount: 50, hold: false, period: "daily" },
        priority: 0,
        directive: "template",
      },
    ]);
  ...
  it("refill with simple limit companion", () => {
    const def = JSON.stringify([
      ...
      {
        type: "simple",
        limit: { amount: 250, hold: false, period: "monthly" },
        priority: 0,
        directive: "template",
      },
    ]);
...
describe("templateToNoteLine", () => {
  ...
  it("periodic with starting date and limit", () => {
    ...
    expect(templateToNoteLine(t)).toBe(
      "#template 100 repeat every 2 weeks starting 2026-01-01 up to 500",
    );
  });
