...
describe("calculateGoal — #goal", () => {
  it("preserves existing budget, sets goal and longGoal", async () => {
    const templates: Template[] = [{ type: "goal", amount: 5000, directive: "goal" }];
    const result = await calculateGoal(
      "cat-1",
      "2026-03",
      templates,
      ctx({ previouslyBudgeted: 15000 }),
    );
  ...
  it("with zero previous budget", async () => {
    const templates: Template[] = [{ type: "goal", amount: 1000, directive: "goal" }];
    const result = await calculateGoal("cat-1", "2026-03", templates, ctx());
    expect(result.budgeted).toBe(0);
...
describe("calculateGoal — average", () => {
  it("averages spent over N months", async () => {
    ...
    mockFirst
      .mockResolvedValueOnce({ spent: -10000 }) // month -1
...
describe("calculateGoal — spend", () => {
  it("distributes remaining across months", async () => {
    ...
    mockFirst
      .mockResolvedValueOnce({ amount: 10000 }) // Jan
  ...
  it("target fully funded → 0", async () => {
    mockFirst.mockResolvedValueOnce({ amount: 30000 }).mockResolvedValueOnce({ amount: 30000 });
...
describe("calculateGoal — remainder", () => {
  it("pass 1: returns hasRemainder marker", async () => {
    const templates: Template[] = [{ type: "remainder", weight: 2, directive: "template" }];
    const result = await calculateGoal("cat-1", "2026-03", templates, ctx());
  ...
  it("pass 2: distributes proportionally by weight", async () => {
    const templates: Template[] = [{ type: "remainder", weight: 3, directive: "template" }];
    const result = await calculateGoal(
      "cat-1",
      "2026-03",
      templates,
    ...
    const templates: Template[] = [
      {
        type: "remainder",
        weight: 1,
        limit: { amount: 100, hold: false, period: "monthly" },
        directive: "template",
      },
    ];
    const result = await calculateGoal(
...
describe("calculateGoal — goal value correctness", () => {
  ...
  it("refill-only (simple no monthly + limit): longGoal true, goal = limit", async () => {
    const templates: Template[] = [
      {
        type: "simple",
        limit: { amount: 1200, hold: false, period: "monthly" },
        priority: 0,
        directive: "template",
      },
    ];
