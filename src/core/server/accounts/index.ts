...
export type AccountGroup = {
  type: "budget" | "offbudget" | "closed";
  label: string;
  total: number;
  accounts: Account[];
};

export function groupAccounts(accounts: Account[], includeClosed = false): AccountGroup[] {
  ...
  if (includeClosed && closed.length > 0) {
    groups.push({
      type: "closed",
      label: "Closed Accounts",
      total: closed.reduce((sum, a) => sum + (a.balance ?? 0), 0),
      accounts: closed,
    });
  }
  return groups;
...
export function getClosedAccountCount(accounts: Account[]): number {
  return accounts.filter((a) => a.closed).length;
}
