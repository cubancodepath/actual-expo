// @ts-nocheck
// FAITHFUL VERBATIM PORT of loot-core src/server/aql/compiler.ts.
// Upstream marks this file `@ts-strict-ignore` (opts out of strict typing); we
// mirror that with `@ts-nocheck` so the 1222-line compiler stays byte-for-byte
// identical to the battle-tested source — hand-typing its expression-walking
// internals would risk subtle compiler bugs. The ONLY intentional deviations
// from upstream are: (1) the two import paths above, and (2) the injectable SQL
// `dialect` (see `./dialect`) that lets expo-sqlite emit built-in SQL functions
// instead of the custom UNICODE_*/NORMALISE/REGEXP that better-sqlite3 registers.
import { getNormalisedString } from "@/core/shared/normalisation";
import type { QueryState } from "@/core/shared/query";
import { type SqlDialect, expoDialect, RegexpUnsupportedError } from "./dialect";
...
function castInput(state, expr, type) {
  ...
  if (type === "date") {
    ...
    if (expr.type === "date") {
      ...
      expr2 =
        parseMonth(expr.value) || parseDate(expr.value) || badDateFormat(expr.value, "date-month");
    } else {
...
const compileFunction = saveStack("function", (state, func) => {
  ...
  switch (name) {
    ...
    case "$substr": {
      ...
      const [arg1, arg2, arg3] = valArray(state, args, ["string", "integer", "integer"]);
      return typed(`SUBSTR(${arg1}, ${arg2}, ${arg3})`, "string");
    }
    case "$lower": {
      validateArgLength(args, 1);
      const [arg1] = valArray(state, args, ["string"]);
      return typed(state.dialect.lower(arg1), "string");
    }
    ...
    case "$nocase":
      validateArgLength(args, 1);
      const [arg1] = valArray(state, args, ["string"]);
      return typed(state.dialect.nocase(arg1), args[0].type);
...
const compileOp = saveStack("op", (state, fieldRef, opData) => {
  ...
  switch (op) {
    ...
    case "$oneof": {
      ...
      return `${String(left)} IN (` + ids.map((id) => `'${String(id)}'`).join(",") + ")";
    }
    case "$like": {
      const [left, right] = valArray(state, [lhs, rhs], ["string", "string"]);
      return state.dialect.like(getNormalisedString(right), left);
    }
    case "$regexp": {
      const [left, right] = valArray(state, [lhs, rhs], ["string", "string"]);
      if (state.dialect.regexp == null) {
        throw new RegexpUnsupportedError();
      }
      return state.dialect.regexp(right, left);
    }
    case "$notlike": {
      const [left, right] = valArray(state, [lhs, rhs], ["string", "string"]);
      return state.dialect.notlike(getNormalisedString(right), left);
    }
    default:
...
export function compileQuery(
  ...
  dialect: SqlDialect = expoDialect,
) {
