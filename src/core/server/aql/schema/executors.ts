// @ts-nocheck
// Faithful verbatim port of loot-core src/server/aql/schema/executors.ts.
// See ../compiler.ts for why the verbatim AQL internals use @ts-nocheck.
...
import { whereIn } from "@/core/server/db/util";
import { q } from "@/core/shared/query";
import type { QueryState } from "@/core/shared/query";
import type { CategoryEntity } from "@/core/types/models";
...
function execTransactions(
  ...
  const splitType = tableOptions.splits ? (tableOptions.splits as string) : "inline";
...
async function execCategoryGroups(
  ...
  const tableOptions = queryState.tableOptions || {};
  const categoriesOption = tableOptions.categories ? (tableOptions.categories as string) : "all";
...
function isValidCategoriesOption(categories: string): categories is CategoriesOption {
