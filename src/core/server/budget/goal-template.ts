/**
 * Goal templates — applying templates to a month's categories and persisting
 * goal_def / goal results. Mirrors loot-core/src/server/budget/goal-template.ts;
...
export async function setGoalResult(
  ...
  const { setBudgetGoal } = await import("./actions");
