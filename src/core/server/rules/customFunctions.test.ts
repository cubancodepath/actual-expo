...
import type { FormulaQueryContext } from "@/core/shared/formulas/customFunctions";
...
function evaluateFormula(formula: string, formulaQueryContext: FormulaQueryContext) {
  const { cellValue } = evaluateFormulaSheet(formula, formulaQueryContext);
  return cellValue;
}

function evaluateFormulaSheet(formula: string, formulaQueryContext: FormulaQueryContext) {
...
describe("CustomFunctionsPlugin formula query functions", () => {
  ...
  it("records QUERY and QUERY_COUNT requests and returns prefetched values", () => {
    ...
    const result = evaluateFormula('=QUERY("income") + QUERY_COUNT("income")', context);
  ...
  it("defaults missing query prefetch values to zero while collecting names", () => {
    ...
    const result = evaluateFormula('=QUERY("missing") + QUERY_COUNT("missing")', context);
  ...
  it("resolves BUDGET_QUERY from prefetched nested query extraction values", () => {
    ...
    context.queryExtractTimeframeStartPrefetch.set("expenses", request.startMonth);
    ...
    expect(context.queryExtractTimeframeStartNames).toEqual(new Set(["expenses"]));
    expect(context.queryExtractTimeframeEndNames).toEqual(new Set(["expenses"]));
  ...
  it("spills QUERY_EXTRACT_CATEGORIES as a standalone array result", () => {
    ...
    const result = evaluateFormulaSheet('=QUERY_EXTRACT_CATEGORIES("expenses")', context);
  ...
  it("surfaces BUDGET_QUERY prefetch failures as formula errors", () => {
    ...
    context.queryExtractTimeframeStartPrefetch.set("expenses", request.startMonth);
    context.queryExtractTimeframeEndPrefetch.set("expenses", request.endMonth);
    context.budgetQueryErrors.set(budgetKey, "Invalid BUDGET_QUERY dimension: spnt");
