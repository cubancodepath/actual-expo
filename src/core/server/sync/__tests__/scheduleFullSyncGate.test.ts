...
import { sendMessages, resetBatchState } from "@/core/server/sync/batch";
import { setSyncingMode } from "@/core/server/sync/syncMode";
import { getSyncTimeout, clearSyncTimeout } from "@/core/server/sync/lifecycle";
import { Timestamp } from "@/core/crdt";

const mocks = vi.hoisted(() => ({ fullSync: vi.fn(async () => 0) }));
vi.mock("@/core/server/sync/fullSync", () => ({
