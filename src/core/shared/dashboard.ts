/**
 * Default dashboard layout — ported verbatim from loot-core
 * `shared/dashboard.ts`. Rendered read-only on mobile when a budget has no
 * `dashboard` rows yet (never written to the table).
 */
import type { NewDashboardWidgetEntity } from "@/core/types/models/dashboard";
...
export const DEFAULT_DASHBOARD_STATE: NewDashboardWidgetEntity[] = [
  // Top row: Key metrics at a glance
  {
    ...
    meta: {
      ...
      content: JSON.stringify({ type: "sum", fontSize: 20 }),
  ...
  },
  {
    ...
    meta: {
      name: "Avg Per Month",
      content: JSON.stringify({ type: "avgPerMonth", fontSize: 20 }),
      timeFrame: { start: "2024-01-01", end: "2024-12-31", mode: "yearToDate" },
      conditions: [
        { field: "amount", op: "lt", value: 0 },
        { field: "account", op: "onBudget", value: "" },
  ...
  },
  {
    ...
    meta: {
      ...
      content: JSON.stringify({ type: "avgPerTransact", fontSize: 20 }),
      timeFrame: { start: "2024-01-01", end: "2024-12-31", mode: "yearToDate" },
      conditions: [
        { field: "amount", op: "lt", value: 0 },
        { field: "account", op: "onBudget", value: "" },
  ...
  // Third row: Spending comparisons
  {
    ...
    x: 0,
    y: 5,
    meta: { name: "This Month", mode: "single-month" },
  },
  {
    type: "spending-card",
    width: 4,
    ...
    x: 4,
    y: 5,
    meta: { name: "Budget Overview", mode: "budget" },
  },
  {
    type: "spending-card",
    width: 4,
    ...
    x: 8,
    y: 5,
    meta: { name: "3-Month Average", mode: "average" },
  },
  ...
  },
  {
    ...
    meta: {
      ...
      content: JSON.stringify({ type: "sum", fontSize: 32 }),
      timeFrame: { start: "2024-01-01", end: "2024-03-31", mode: "sliding-window" },
      conditions: [],
      conditionsOp: "and",
    },
