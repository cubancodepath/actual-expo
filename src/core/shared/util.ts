...
// numbers are so large that it's not safe for us — we give division more
// room to be correct.
const MAX_SAFE_NUMBER = 2 ** 51 - 1;
const MIN_SAFE_NUMBER = -MAX_SAFE_NUMBER;

/**
 * Budget arithmetic is done entirely in integer cents. Any result that isn't an
 * integer, or overflows the safe range for exact float math, signals a bug in
 * the formula that produced it — throwing here turns a silent, cascading
 * wrong-balance bug into an immediate, loud one at the cell that produced it.
 */
