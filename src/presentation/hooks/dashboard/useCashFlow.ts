import { useMemo, useState } from "react";
import { q } from "@core/queries";
import { useLiveQuery } from "../useQuery";
import { addMonths } from "@/lib/date";

export type CashFlowPoint = {
  month: string;
  income: number;
  expenses: number;
};

export type CashFlowRange = 3 | 6 | 12;

const SHORT_MONTHS = [
  "Jan",
  "Feb",
  "Mar",
  "Apr",
  "May",
  "Jun",
  "Jul",
  "Aug",
  "Sep",
  "Oct",
  "Nov",
  "Dec",
];

function monthLabel(month: string): string {
  const m = parseInt(month.slice(5), 10);
  return SHORT_MONTHS[m - 1] ?? month.slice(5);
}

/** Month boundaries as YYYYMMDD integers. */
function monthRange(month: string): { start: number; end: number } {
  const [y, m] = month.split("-").map(Number);
  const lastDay = new Date(y, m, 0).getDate();
  return {
    start: y * 10000 + m * 100 + 1,
    end: y * 10000 + m * 100 + lastDay,
  };
}

type MonthCashFlow = { income: number; expenses: number };

/** Income + expenses for a single month. */
function useMonthCashFlow(monthStr: string): MonthCashFlow {
  const { start, end } = useMemo(() => monthRange(monthStr), [monthStr]);

  const { data: incomeData } = useLiveQuery<{ result: number }>(
    () =>
      q("transactions")
        .filter({ date: { $gte: start, $lte: end }, amount: { $gt: 0 }, transferred_id: null })
        .calculate({ $sum: "$amount" }),
    [start, end],
  );

  const { data: expenseData } = useLiveQuery<{ result: number }>(
    () =>
      q("transactions")
        .filter({ date: { $gte: start, $lte: end }, amount: { $lt: 0 }, transferred_id: null })
        .calculate({ $sum: "$amount" }),
    [start, end],
  );

  return useMemo(
    () => ({
      income: incomeData?.[0]?.result ?? 0,
      expenses: expenseData?.[0]?.result ?? 0,
    }),
    [incomeData, expenseData],
  );
}

/**
 * Cash flow (income vs expenses) for multiple months.
 * Always runs 12 hook pairs (stable count) and slices based on range.
 */
export function useCashFlow() {
  const [range, setRange] = useState<CashFlowRange>(6);

  // Current month from date (not from budget store — cash flow always uses real date)
  const now = new Date();
  const currentMonth = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, "0")}`;

  // 12 month strings
  const m11 = useMemo(() => addMonths(currentMonth, -11), [currentMonth]);
  const m10 = useMemo(() => addMonths(currentMonth, -10), [currentMonth]);
  const m9 = useMemo(() => addMonths(currentMonth, -9), [currentMonth]);
  const m8 = useMemo(() => addMonths(currentMonth, -8), [currentMonth]);
  const m7 = useMemo(() => addMonths(currentMonth, -7), [currentMonth]);
  const m6 = useMemo(() => addMonths(currentMonth, -6), [currentMonth]);
  const m5 = useMemo(() => addMonths(currentMonth, -5), [currentMonth]);
  const m4 = useMemo(() => addMonths(currentMonth, -4), [currentMonth]);
  const m3 = useMemo(() => addMonths(currentMonth, -3), [currentMonth]);
  const m2 = useMemo(() => addMonths(currentMonth, -2), [currentMonth]);
  const m1 = useMemo(() => addMonths(currentMonth, -1), [currentMonth]);
  const m0 = currentMonth;

  // 12 stable hook calls
  const v11 = useMonthCashFlow(m11);
  const v10 = useMonthCashFlow(m10);
  const v9 = useMonthCashFlow(m9);
  const v8 = useMonthCashFlow(m8);
  const v7 = useMonthCashFlow(m7);
  const v6 = useMonthCashFlow(m6);
  const v5 = useMonthCashFlow(m5);
  const v4 = useMonthCashFlow(m4);
  const v3 = useMonthCashFlow(m3);
  const v2 = useMonthCashFlow(m2);
  const v1 = useMonthCashFlow(m1);
  const v0 = useMonthCashFlow(m0);

  const allMonths = [m11, m10, m9, m8, m7, m6, m5, m4, m3, m2, m1, m0];
  const allValues = [v11, v10, v9, v8, v7, v6, v5, v4, v3, v2, v1, v0];

  const trend: CashFlowPoint[] = useMemo(() => {
    const start = 12 - range;
    const points: CashFlowPoint[] = [];
    for (let i = start; i < 12; i++) {
      points.push({
        month: monthLabel(allMonths[i]),
        income: allValues[i].income / 100,
        expenses: Math.abs(allValues[i].expenses) / 100,
      });
    }
    return points;
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [
    range,
    v11.income,
    v11.expenses,
    v10.income,
    v10.expenses,
    v9.income,
    v9.expenses,
    v8.income,
    v8.expenses,
    v7.income,
    v7.expenses,
    v6.income,
    v6.expenses,
    v5.income,
    v5.expenses,
    v4.income,
    v4.expenses,
    v3.income,
    v3.expenses,
    v2.income,
    v2.expenses,
    v1.income,
    v1.expenses,
    v0.income,
    v0.expenses,
    m11,
    m10,
    m9,
    m8,
    m7,
    m6,
    m5,
    m4,
    m3,
    m2,
    m1,
    m0,
  ]);

  // Current month values for summary
  const income = v0.income;
  const expenses = v0.expenses;
  const net = income + expenses;

  // Previous month for comparison
  const prevNet = v1.income + v1.expenses;

  return {
    income,
    expenses,
    net,
    prevNet,
    trend,
    range,
    setRange,
    isLoading: false,
  };
}
