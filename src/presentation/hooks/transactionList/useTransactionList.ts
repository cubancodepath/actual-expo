...
export function useTransactionList({
  ...
  const loadMore = useCallback(async () => {
    if (stateRef.current.loading || stateRef.current.loadingMore || !stateRef.current.hasMore)
      return;
      ...
      { text: "Cancel", style: "cancel" },
      {
        ...
        onPress: async () => {
          ...
          queueMicrotask(() =>
            useUndoStore
              .getState()
              .showUndo(`${ids.size} transaction${ids.size === 1 ? "" : "s"} deleted`),
          );
        },
      },
    ]);
        ...
        { text: "Cancel", style: "cancel" },
        {
          ...
          onPress: async () => {
            ...
            if (optimisticBulkMoveRef.current) {
              const updated = optimisticBulkMoveRef.current(
  ...
  const handleBulkChangeCategory = useCallback(
    async (categoryId: string | null, categoryName?: string) => {
      ...
      dispatch({
        type: "BULK_UPDATE",
        ids,
        fields: { category: categoryId, categoryName: categoryName ?? null },
      });
      dispatch({ type: "EXIT_SELECT_MODE" });
      onExitRef.current?.();
      Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
  ...
  useEffect(() => {
    ...
    if (pendingCategoryRef.current) {
      ...
      dispatch({
        type: "UPDATE",
        txnId,
        fields: { category: selectedCategory.id, categoryName: selectedCategory.name },
      });
