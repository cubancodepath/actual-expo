...
describe('evalCondition', () => {
  ...
  describe('is operator', () => {
    ...
    it('matches date with YYYY (year precision)', () => {
      expect(
        evalCondition(
          { field: 'date', op: 'is', value: '2024', type: 'date' },
          { date: 20240315 },
        ),
      ).toBe(true);
    });
  ...
  describe('isNot operator', () => {
    it('returns true for different strings', () => {
      expect(
        evalCondition(
          { field: 'notes', op: 'isNot', value: 'groceries' },
          { notes: 'pharmacy' },
        ),
      ).toBe(true);
    });
  ...
  describe('matches operator', () => {
    ...
    it('returns false for invalid regex', () => {
      expect(
        evalCondition(
          { field: 'notes', op: 'matches', value: '[invalid' },
          { notes: 'test' },
        ),
      ).toBe(false);
    });
  ...
  describe('isbetween operator', () => {
    it('matches value within range', () => {
      expect(
        evalCondition(
          { field: 'amount', op: 'isbetween', value: { num1: -10000, num2: -5000 }, type: 'number' },
          { amount: -7500 },
        ),
      ).toBe(true);
    });
  ...
  describe('gt/gte/lt/lte operators', () => {
    it('gt matches greater values', () => {
      expect(
        evalCondition(
          { field: 'amount', op: 'gt', value: 100, type: 'number' },
          { amount: 200 },
        ),
      ).toBe(true);
    });
    ...
    it('gte matches equal values', () => {
      expect(
        evalCondition(
          { field: 'amount', op: 'gte', value: 100, type: 'number' },
          { amount: 100 },
        ),
      ).toBe(true);
    });
    ...
    it('lt matches lesser values', () => {
      expect(
        evalCondition(
          { field: 'amount', op: 'lt', value: 100, type: 'number' },
          { amount: 50 },
        ),
      ).toBe(true);
    });
    ...
    it('lte matches equal values', () => {
      expect(
        evalCondition(
          { field: 'amount', op: 'lte', value: 100, type: 'number' },
          { amount: 100 },
        ),
      ).toBe(true);
    });
...
describe('execAction', () => {
  ...
  it('set-split-amount with fixed-amount sets amount', () => {
    ...
    execAction(
      { op: 'set-split-amount', value: -5000, options: { method: 'fixed-amount' } },
      txn,
    );
