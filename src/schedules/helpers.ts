...
export function extractScheduleConds(conditions: RuleCondition[]) {
  return {
    ...
    amount:
      conditions.find(
        c =>
          (c.op === 'is' || c.op === 'isapprox' || c.op === 'isbetween') &&
          c.field === 'amount',
      ) ?? null,
    date:
      conditions.find(
        c => (c.op === 'is' || c.op === 'isapprox') && c.field === 'date',
      ) ?? null,
  };
}
...
export function getRecurringDescription(config: RecurConfig): string {
  ...
  switch (config.frequency) {
    ...
    case 'yearly': {
      ...
      desc =
        interval !== 1
          ? `Every ${interval} years on ${dateStr}`
          : `Every year on ${dateStr}`;
      break;
    }
    default:
