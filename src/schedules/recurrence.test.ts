...
describe('occursBetween', () => {
  ...
  it('occurrence within range → true', () => {
    expect(
      occursBetween(config, parseDate('2026-03-09'), parseDate('2026-03-15')),
    ).toBe(true);
  });
  ...
  it('no occurrence in range → false', () => {
    ...
    expect(
      occursBetween(config, parseDate('2026-03-10'), parseDate('2026-03-14')),
    ).toBe(false);
  });

  it('occurrence on boundary → true', () => {
    expect(
      occursBetween(config, parseDate('2026-03-09'), parseDate('2026-03-09')),
    ).toBe(true);
  });
});
